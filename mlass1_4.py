...
from numpy import zeros, e, array, asarray, log, ones, sum, shape, c_, r_, append, mean, cov, diag, linalg, cumsum, argsort, vstack, argmax
...
grad = 1/m *(zsigmoid(X*theta)-Y)'*X + [0,lambda/m * theta(2:end)'];
...
def grad(theta, X, y, l):
    print('g1', shape(theta), shape(X), y, l)
    z = X.dot(theta)
    print('g2', shape(l / m * theta[1:]))
    print('g3', shape(r_[0, l / m * theta[1:]]))
    print('g4', shape(1 / m * (sigmoid(z)-y).T.dot(X)))
...
def class_weights(X, y, num_cl, l):
    ...
    for c in range(num_cl):
        y2 = asarray(y == c)
#        print(fmin_l_bfgs_b(cost_function_reg, x0=theta_initial, args=(X, y, l), fprime=grad))
#        print(fmin_l_bfgs_b(cost, x0=theta_initial, args=(X, y, l), fprime=grad))
