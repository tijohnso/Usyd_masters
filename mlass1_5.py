...
from numpy import zeros, e, array, asarray, log, ones, sum, shape, c_, r_, append, mean, cov, diag, linalg, cumsum, argsort, vstack, argmax, subtract
...
#    grad = (1 / m) * (sigmoid(z) - y).T.dot(X) + r_[0, (l / m) * theta[1:].T]
    grad = (1 / m) * (sigmoid(z) - y)[:,0].T.dot(X) + r_[0, (l / m) * theta[1:].T]
#    print('g10', (sigmoid(z) - y)[0,:])
#    print('g10b', shape((sigmoid(z) - y)[0,:]))
#    print('g11', (sigmoid(z) - y))
#    print('g11b', shape((sigmoid(z) - y)))
#    print('g5', shape(np.subtract(sigmoid(z), y)))
#    print('g7', shape(grad))
    return grad
