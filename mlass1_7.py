...
#Predictions. Pick the column index with the best probability for each row of the probability array
for i in shape(prob[:,0]):
    predictions = [argmax(prob[j]) for j in range(len(prob))]
predictions = array(predictions).reshape(20104,1)

#Compare predictions to train_y
compare_predicted = array([train_y[i] == predictions[i] for i in range(len(train_y))])
num_correctly_predicted = compare_predicted.T.dot(ones(shape(train_y)))

accuracy = num_correctly_predicted / len(train_y)
#0.5870971
